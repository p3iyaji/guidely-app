...
trait ValidatesSettingTermFields
{
    protected function prepareSettingTermFields(): void
    {
        if ($this->exists('code')) {
            $this->merge([
     ...
     */
    protected function settingTermRules(bool $partial): array
    {
        $presence = $partial ? 'sometimes' : 'required';
        $versionId = app(EffectiveOntologyVersion::class)->id();
        $ignore = $this->route('settingTerm');
        $ignoreId = $ignore instanceof SettingTerm ? $ignore->id : null;

        $unique = Rule::unique('setting_terms', 'code')
     ...
     */
    protected function settingTermMessages(): array
    {
        return [
            'code.regex' => 'The code may contain only letters, numbers, hyphens, and underscores.',
            'code.unique' => 'A Setting term with this code already exists on this Ontology version.',
        ];
    }
     ...
     * @return list<\Closure>
     */
    protected function settingTermAfterHooks(): array
    {
        return [
