<template>
    <div data-testid="threshold-terms-page">
        <PageHero
            eyebrow="Configuration"
            title="Threshold terms"
            :description="pageIntro"
        >
            <template v-if="canManage && !loading && !loadError" #action>
                <ButtonPrimary
                    data-testid="threshold-terms-add-open"
        ...
        <CrudSearch
            id="threshold-terms-search"
            v-model="searchQuery"
            placeholder="Search by code or label"
            test-id="threshold-terms-search"
        />

        <div v-if="loading" class="space-y-3" data-testid="threshold-terms-loading">
        ...
        <p
            ...
            data-testid="threshold-terms-error"
            role="alert"
        >
            {{ loadError }}
        </p>
        ...
        <template v-else>
            <EmptyState
                v-if="terms.length === 0"
                test-id="threshold-terms-empty"
            >
                No Threshold terms on this Ontology version.
                <template v-if="canManage" #actions>
                    <ButtonPrimary
                        data-testid="threshold-terms-add-cta"
                        @click="openCreateForm"
                    >
                        Add Threshold term
                    </ButtonPrimary>
            ...
            <EmptyState
                ...
                test-id="threshold-terms-search-empty"
            >
                No Threshold terms match your search.
            </EmptyState>

            <DataTable
                v-else-if="filteredTerms.length > 0"
                test-id="threshold-terms-list"
            >
                <template #head>
                    <tr>
                ...
                <tr
                    ...
                    data-testid="threshold-term-row"
                >
                    <td class="px-4 py-3 font-medium text-text" data-testid="threshold-term-label">
                        {{ term.label }}
                    </td>
                    <td class="px-4 py-3 font-mono text-meta text-text-muted" data-testid="threshold-term-code">
                        {{ term.code }}
                    </td>
                    <td class="px-4 py-3" data-testid="threshold-term-status">
                    ...
                    <td class="px-4 py-3 text-right">
                        <div v-if="canManage" class="flex flex-wrap justify-end gap-2">
                            <TableAction
                                ...
                                :data-testid="`threshold-term-edit-${term.id}`"
                            ...
                            <TableAction
                                ...
                                tone="danger"
                                :data-testid="`threshold-term-delete-${term.id}`"
        ...
        <Modal
            ...
            :title="formMode === 'edit' ? 'Edit Threshold term' : 'Add Threshold term'"
            :close-disabled="saving"
            data-testid="threshold-terms-form"
            @close="closeForm"
        >
            <p class="text-body text-text-muted">
                Inactive terms stay listed here but are hidden from pickers.
            </p>

            <form class="mt-4 space-y-4" @submit.prevent="submitTermForm">
                <div>
                    <label class="block text-body text-text" for="threshold-term-code">Code</label>
                    <input
                        id="threshold-term-code"
                        ...
                        data-testid="threshold-term-code-input"
                    >
                    <p
                ...
                <div>
                    <label class="block text-body text-text" for="threshold-term-label">Label</label>
                    <input
                        id="threshold-term-label"
                        v-model="termForm.label"
                        type="text"
                        required
                        maxlength="255"
                        class="mt-1 w-full rounded-md border border-border bg-surface px-3 py-2 text-body text-text focus:outline-none focus:ring-2 focus:ring-focus-ring"
                        data-testid="threshold-term-label-input"
                    >
                    <p
                ...
                <div>
                    <label class="block text-body text-text" for="threshold-term-sort-order">Sort order</label>
                    <input
                        id="threshold-term-sort-order"
                        v-model.number="termForm.sort_order"
                        type="number"
                        min="0"
                        max="65535"
                        class="mt-1 w-full rounded-md border border-border bg-surface px-3 py-2 text-body text-text focus:outline-none focus:ring-2 focus:ring-focus-ring"
                        data-testid="threshold-term-sort-order-input"
                    >
                    <p
                ...
                <label class="flex items-center gap-2 text-body text-text">
                    <input
                        ...
                        type="checkbox"
                        data-testid="threshold-term-active-input"
                    >
                    Active Threshold term
                </label>
                <p
                ...
                <p
                    ...
                    data-testid="threshold-terms-form-error"
                    role="alert"
                >
                    {{ formError }}
                </p>
                <div class="flex flex-wrap gap-3">
                    <ButtonPrimary
                        type="submit"
                        :disabled="saving"
                        data-testid="threshold-terms-form-submit"
                    >
                        {{ saving ? 'Saving…' : (formMode === 'create' ? 'Save Threshold term' : 'Save changes') }}
                    </ButtonPrimary>
                    <ButtonSecondary
                        :disabled="saving"
                        data-testid="threshold-terms-form-cancel"
                        @click="closeForm"
                    >
                        Cancel
                    </ButtonSecondary>
        ...
        <Modal
            ...
            title="Delete Threshold term"
            :close-disabled="saving"
            data-testid="threshold-terms-delete-confirm"
            @close="closeForm"
        >
            <p class="text-body text-text">
                Delete {{ selectedTerm?.label }}? Terms used in relationship mappings cannot be deleted.
            </p>
            <p
                v-if="formError"
            ...
            <div class="mt-4 flex flex-wrap gap-3">
                <ButtonPrimary
                    :disabled="saving"
                    data-testid="threshold-terms-delete-confirm-submit"
                    @click="submitDelete"
                >
                    {{ saving ? 'Deleting…' : 'Delete Threshold term' }}
                </ButtonPrimary>
                <ButtonSecondary
                    :disabled="saving"
                    data-testid="threshold-terms-delete-cancel"
                    @click="closeForm"
                >
                    Cancel
                </ButtonSecondary>
...
const pageIntro = computed(() => {
    if (canManage.value) {
        return 'Create, update, and remove Threshold terms for this Ontology version. Inactive terms remain listed here.';
    }

    return 'Threshold terms for this Tenant’s Ontology version. Creating and editing terms is limited to Tenant Admins.';
});
...
async function loadTerms() {
    ...
    try {
        const response = await apiFetch('/api/v1/ontology/threshold-terms');

        if (!response.ok) {
            loadError.value = 'Unable to load Threshold terms.';
            terms.value = [];
...
async function submitTermForm() {
    ...
    const url = isCreate
        ...
        : `/api/v1/ontology/threshold-terms/${selectedTerm.value?.id}`;
    ...
    try {
        ...
        if (!response.ok) {
            ...
            formError.value = payload.message ?? (isCreate ? 'Unable to create Threshold term.' : 'Unable to update Threshold term.');
        ...
        formError.value = isCreate ? 'Unable to create Threshold term.' : 'Unable to update Threshold term.';
    } finally {
        saving.value = false;
    }
}
...
async function submitDelete() {
    ...
    try {
        const response = await apiFetch(`/api/v1/ontology/threshold-terms/${termId}`, {
            method: 'DELETE',
        });

        if (!response.ok) {
            const payload = await response.json().catch(() => ({}));
            formError.value = payload.message ?? 'Unable to delete Threshold term.';
        ...
        formError.value = 'Unable to delete Threshold term.';
    } finally {
        saving.value = false;
    }
}
