<template>
    <div data-testid="outcome-terms-page">
        <PageHero
        ...
        >
            <template v-if="canManage && !loading && !loadError" #action>
                <ButtonPrimary
                    data-testid="outcome-terms-add-open"
        ...
        <CrudSearch
            id="outcome-terms-search"
            v-model="searchQuery"
            placeholder="Search by code or label"
            test-id="outcome-terms-search"
        />

        <div v-if="loading" class="space-y-3" data-testid="outcome-terms-loading">
        ...
        <p
            ...
            data-testid="outcome-terms-error"
            role="alert"
        >
            {{ loadError }}
        </p>
        ...
        <template v-else>
            <EmptyState
                v-if="terms.length === 0"
                test-id="outcome-terms-empty"
            >
                No Outcome terms on this Ontology version.
                <template v-if="canManage" #actions>
                    <ButtonPrimary
                        data-testid="outcome-terms-add-cta"
                        @click="openCreateForm"
                    >
                        Add Outcome term
                    </ButtonPrimary>
            ...
            <EmptyState
                ...
                test-id="outcome-terms-search-empty"
            >
                No Outcome terms match your search.
            </EmptyState>

            <DataTable
                v-else-if="filteredTerms.length > 0"
                test-id="outcome-terms-list"
            >
                <template #head>
                    <tr>
                ...
                <tr
                    ...
                    data-testid="outcome-term-row"
                >
                    <td class="px-4 py-3 font-medium text-text" data-testid="outcome-term-label">
                        {{ term.label }}
                    </td>
                    <td class="px-4 py-3 font-mono text-meta text-text-muted" data-testid="outcome-term-code">
                        {{ term.code }}
                    </td>
                    <td class="px-4 py-3" data-testid="outcome-term-status">
                    ...
                    <td class="px-4 py-3 text-right">
                        <div v-if="canManage" class="flex flex-wrap justify-end gap-2">
                            <TableAction
                                ...
                                :data-testid="`outcome-term-edit-${term.id}`"
                            ...
                            <TableAction
                                ...
                                tone="danger"
                                :data-testid="`outcome-term-delete-${term.id}`"
        ...
        <Modal
            ...
            :title="formMode === 'edit' ? 'Edit Outcome term' : 'Add Outcome term'"
            :close-disabled="saving"
            data-testid="outcome-terms-form"
            @close="closeForm"
        >
            ...
            <form class="mt-4 space-y-4" @submit.prevent="submitTermForm">
                <div>
                    <label class="block text-body text-text" for="outcome-term-code">Code</label>
                    <input
                        id="outcome-term-code"
                        ...
                        data-testid="outcome-term-code-input"
                    >
                    <p
                ...
                <div>
                    <label class="block text-body text-text" for="outcome-term-label">Label</label>
                    <input
                        id="outcome-term-label"
                        v-model="termForm.label"
                        type="text"
                        required
                        maxlength="255"
                        class="mt-1 w-full rounded-md border border-border bg-surface px-3 py-2 text-body text-text focus:outline-none focus:ring-2 focus:ring-focus-ring"
                        data-testid="outcome-term-label-input"
                    >
                    <p
                ...
                <div>
                    <label class="block text-body text-text" for="outcome-term-sort-order">Sort order</label>
                    <input
                        id="outcome-term-sort-order"
                        v-model.number="termForm.sort_order"
                        type="number"
                        min="0"
                        max="65535"
                        class="mt-1 w-full rounded-md border border-border bg-surface px-3 py-2 text-body text-text focus:outline-none focus:ring-2 focus:ring-focus-ring"
                        data-testid="outcome-term-sort-order-input"
                    >
                    <p
                ...
                <label class="flex items-center gap-2 text-body text-text">
                    <input
                        ...
                        type="checkbox"
                        data-testid="outcome-term-active-input"
                    >
                    Active Outcome term
                </label>
                <p
                ...
                <p
                    ...
                    data-testid="outcome-terms-form-error"
                    role="alert"
                >
                    {{ formError }}
                </p>
                <div class="flex flex-wrap gap-3">
                    <ButtonPrimary
                        type="submit"
                        :disabled="saving"
                        data-testid="outcome-terms-form-submit"
                    >
                        {{ saving ? 'Saving…' : (formMode === 'create' ? 'Save Outcome term' : 'Save changes') }}
                    </ButtonPrimary>
                    <ButtonSecondary
                        :disabled="saving"
                        data-testid="outcome-terms-form-cancel"
                        @click="closeForm"
                    >
                        Cancel
                    </ButtonSecondary>
        ...
        <Modal
            ...
            title="Delete Outcome term"
            :close-disabled="saving"
            data-testid="outcome-terms-delete-confirm"
            @close="closeForm"
        >
            ...
            <div class="mt-4 flex flex-wrap gap-3">
                <ButtonPrimary
                    :disabled="saving"
                    data-testid="outcome-terms-delete-confirm-submit"
                    @click="submitDelete"
                >
                    {{ saving ? 'Deleting…' : 'Delete Outcome term' }}
                </ButtonPrimary>
                <ButtonSecondary
                    :disabled="saving"
                    data-testid="outcome-terms-delete-cancel"
                    @click="closeForm"
                >
                    Cancel
                </ButtonSecondary>
...
const pageIntro = computed(() => {
    if (canManage.value) {
        return 'Create, update, and remove Outcome terms for this Ontology version. Inactive terms remain listed here.';
    }

    return 'Outcome terms for this Tenant’s Ontology version. Creating and editing terms is limited to Tenant Admins.';
});
...
async function loadTerms() {
    ...
    try {
        const response = await apiFetch('/api/v1/ontology/outcome-terms');

        if (!response.ok) {
            loadError.value = 'Unable to load Outcome terms.';
            terms.value = [];
...
async function submitTermForm() {
    ...
    const url = isCreate
        ...
        : `/api/v1/ontology/outcome-terms/${selectedTerm.value?.id}`;
    ...
    try {
        ...
        if (!response.ok) {
            ...
            formError.value = payload.message ?? (isCreate ? 'Unable to create Outcome term.' : 'Unable to update Outcome term.');
        ...
        formError.value = isCreate ? 'Unable to create Outcome term.' : 'Unable to update Outcome term.';
    } finally {
        saving.value = false;
    }
}
...
async function submitDelete() {
    ...
    try {
        const response = await apiFetch(`/api/v1/ontology/outcome-terms/${termId}`, {
            method: 'DELETE',
        });

        if (!response.ok) {
            const payload = await response.json().catch(() => ({}));
            formError.value = payload.message ?? 'Unable to delete Outcome term.';
        ...
        formError.value = 'Unable to delete Outcome term.';
    } finally {
        saving.value = false;
    }
}
