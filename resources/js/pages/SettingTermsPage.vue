<template>
    <div data-testid="setting-terms-page">
        <PageHero
        ...
        >
            <template v-if="canManage && !loading && !loadError" #action>
                <ButtonPrimary
                    data-testid="setting-terms-add-open"
        ...
        <CrudSearch
            id="setting-terms-search"
            v-model="searchQuery"
            placeholder="Search by code or label"
            test-id="setting-terms-search"
        />

        <div v-if="loading" class="space-y-3" data-testid="setting-terms-loading">
        ...
        <p
            ...
            data-testid="setting-terms-error"
            role="alert"
        >
            {{ loadError }}
        </p>
        ...
        <template v-else>
            <EmptyState
                v-if="terms.length === 0"
                test-id="setting-terms-empty"
            >
                No Setting terms on this Ontology version.
                <template v-if="canManage" #actions>
                    <ButtonPrimary
                        data-testid="setting-terms-add-cta"
                        @click="openCreateForm"
                    >
                        Add Setting term
                    </ButtonPrimary>
            ...
            <EmptyState
                ...
                test-id="setting-terms-search-empty"
            >
                No Setting terms match your search.
            </EmptyState>

            <DataTable
                v-else-if="filteredTerms.length > 0"
                test-id="setting-terms-list"
            >
                <template #head>
                    <tr>
                ...
                <tr
                    ...
                    data-testid="setting-term-row"
                >
                    <td class="px-4 py-3 font-medium text-text" data-testid="setting-term-label">
                        {{ term.label }}
                    </td>
                    <td class="px-4 py-3 font-mono text-meta text-text-muted" data-testid="setting-term-code">
                        {{ term.code }}
                    </td>
                    <td class="px-4 py-3" data-testid="setting-term-status">
                    ...
                    <td class="px-4 py-3 text-right">
                        <div v-if="canManage" class="flex flex-wrap justify-end gap-2">
                            <TableAction
                                ...
                                :data-testid="`setting-term-edit-${term.id}`"
                            ...
                            <TableAction
                                ...
                                tone="danger"
                                :data-testid="`setting-term-delete-${term.id}`"
        ...
        <Modal
            ...
            :title="formMode === 'edit' ? 'Edit Setting term' : 'Add Setting term'"
            :close-disabled="saving"
            data-testid="setting-terms-form"
            @close="closeForm"
        >
            <p class="text-body text-text-muted">
                Inactive terms stay listed here but are hidden from evidence capture pickers.
            </p>

            <form class="mt-4 space-y-4" @submit.prevent="submitTermForm">
                <div>
                    <label class="block text-body text-text" for="setting-term-code">Code</label>
                    <input
                        id="setting-term-code"
                        ...
                        data-testid="setting-term-code-input"
                    >
                    <p
                ...
                <div>
                    <label class="block text-body text-text" for="setting-term-label">Label</label>
                    <input
                        id="setting-term-label"
                        v-model="termForm.label"
                        type="text"
                        required
                        maxlength="255"
                        class="mt-1 w-full rounded-md border border-border bg-surface px-3 py-2 text-body text-text focus:outline-none focus:ring-2 focus:ring-focus-ring"
                        data-testid="setting-term-label-input"
                    >
                    <p
                ...
                <div>
                    <label class="block text-body text-text" for="setting-term-sort-order">Sort order</label>
                    <input
                        id="setting-term-sort-order"
                        v-model.number="termForm.sort_order"
                        type="number"
                        min="0"
                        max="65535"
                        class="mt-1 w-full rounded-md border border-border bg-surface px-3 py-2 text-body text-text focus:outline-none focus:ring-2 focus:ring-focus-ring"
                        data-testid="setting-term-sort-order-input"
                    >
                    <p
                ...
                <label class="flex items-center gap-2 text-body text-text">
                    <input
                        ...
                        type="checkbox"
                        data-testid="setting-term-active-input"
                    >
                    Active Setting term
                </label>
                <p
                ...
                <p
                    ...
                    data-testid="setting-terms-form-error"
                    role="alert"
                >
                    {{ formError }}
                </p>
                <div class="flex flex-wrap gap-3">
                    <ButtonPrimary
                        type="submit"
                        :disabled="saving"
                        data-testid="setting-terms-form-submit"
                    >
                        {{ saving ? 'Saving…' : (formMode === 'create' ? 'Save Setting term' : 'Save changes') }}
                    </ButtonPrimary>
                    <ButtonSecondary
                        :disabled="saving"
                        data-testid="setting-terms-form-cancel"
                        @click="closeForm"
                    >
                        Cancel
                    </ButtonSecondary>
        ...
        <Modal
            ...
            title="Delete Setting term"
            :close-disabled="saving"
            data-testid="setting-terms-delete-confirm"
            @close="closeForm"
        >
            <p class="text-body text-text">
                Delete {{ selectedTerm?.label }}? Terms used in evidence records cannot be deleted.
            </p>
            <p
                v-if="formError"
            ...
            <div class="mt-4 flex flex-wrap gap-3">
                <ButtonPrimary
                    :disabled="saving"
                    data-testid="setting-terms-delete-confirm-submit"
                    @click="submitDelete"
                >
                    {{ saving ? 'Deleting…' : 'Delete Setting term' }}
                </ButtonPrimary>
                <ButtonSecondary
                    :disabled="saving"
                    data-testid="setting-terms-delete-cancel"
                    @click="closeForm"
                >
                    Cancel
                </ButtonSecondary>
...
const pageIntro = computed(() => {
    if (canManage.value) {
        return 'Create, update, and remove Setting terms for evidence capture. Inactive terms remain listed here.';
    }

    return 'Setting terms for this Tenant’s Ontology version. Creating and editing terms is limited to Tenant Admins.';
});
...
async function loadTerms() {
    ...
    try {
        const response = await apiFetch('/api/v1/ontology/setting-terms');

        if (!response.ok) {
            loadError.value = 'Unable to load Setting terms.';
            terms.value = [];
...
async function submitTermForm() {
    ...
    const url = isCreate
        ...
        : `/api/v1/ontology/setting-terms/${selectedTerm.value?.id}`;
    ...
    try {
        ...
        if (!response.ok) {
            ...
            formError.value = payload.message ?? (isCreate ? 'Unable to create Setting term.' : 'Unable to update Setting term.');
        ...
        formError.value = isCreate ? 'Unable to create Setting term.' : 'Unable to update Setting term.';
    } finally {
        saving.value = false;
    }
}
...
async function submitDelete() {
    ...
    try {
        const response = await apiFetch(`/api/v1/ontology/setting-terms/${termId}`, {
            method: 'DELETE',
        });

        if (!response.ok) {
            const payload = await response.json().catch(() => ({}));
            formError.value = payload.message ?? 'Unable to delete Setting term.';
        ...
        formError.value = 'Unable to delete Setting term.';
    } finally {
        saving.value = false;
    }
}
